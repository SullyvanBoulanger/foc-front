...
import React, {
  ChangeEvent,
  Dispatch, ReactElement, SetStateAction, useContext, useState,
} from 'react';
...
export default function SearchBar({ setResult } : SearchBarProps): ReactElement {
  ...
  const { setDisabled, setUrlQuery } = useContext(SearchContext);
  ...
  const onSubmit = ((data: Record<string, PrimitiveTypes>) => {
    setUrlQuery(`/card/search?${constructQuery(data)}`);
    setDisabled(false);
  });
  ...
  const onChange = ((event: ChangeEvent<HTMLInputElement>) => {
    ...
    setDisabled(false);
    setUrlQuery(`/card/search?name=${currentValue}`);
